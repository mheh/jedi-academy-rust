...
pub fn MD4Update(context: *mut MD4_CTX, input: *const u8, inputLen: u32);
pub fn MD4Final(digest: *mut u8, context: *mut MD4_CTX);
...
fn MD4Transform(state: *mut UINT4, block: *const u8);
fn Encode(output: *mut u8, input: *mut UINT4, len: u32);
...
macro_rules! FF {
	($a:ident, $b:expr, $c:expr, $d:expr, $x:expr, $s:expr) => {
...
pub fn MD4Update(context: *mut MD4_CTX, input: *const u8, inputLen: u32)
{
	unsafe {
		...
		if ((*context).count[0].wrapping_add(inputLen << 3)) < (inputLen << 3) {
			(*context).count[1] = (*context).count[1].wrapping_add(1);
		}
		(*context).count[0] = (*context).count[0].wrapping_add(inputLen << 3);
		...
		if inputLen >= partLen
		{
	 		core::ptr::copy_nonoverlapping(input, &mut (*context).buffer[index as usize] as *mut u8, partLen as usize);
	 		MD4Transform((*context).state.as_mut_ptr(), (*context).buffer.as_ptr());

	 		i = partLen;
	 		while i.wrapping_add(63) < inputLen {
	 			MD4Transform((*context).state.as_mut_ptr(), input.add(i as usize));
	 			i = i.wrapping_add(64);
	 		}
		...
		core::ptr::copy_nonoverlapping(input.add(i as usize), &mut (*context).buffer[index as usize] as *mut u8, (inputLen - i) as usize);
	}
}
...
pub fn MD4Final(digest: *mut u8, context: *mut MD4_CTX)
{
	unsafe {
		...
		core::ptr::write_bytes(context as *mut u8, 0, core::mem::size_of::<MD4_CTX>());
	}
}
...
fn MD4Transform(state: *mut UINT4, block: *const u8)
{
	...
	*state.add(0) = state.add(0).read().wrapping_add(a);
	*state.add(1) = state.add(1).read().wrapping_add(b);
	*state.add(2) = state.add(2).read().wrapping_add(c);
	*state.add(3) = state.add(3).read().wrapping_add(d);

		/* Zeroize sensitive information.*/
		core::ptr::write_bytes(x.as_mut_ptr() as *mut u8, 0, core::mem::size_of_val(&x));
	}
}
...
fn Decode(output: *mut UINT4, input: *const u8, len: u32)
{
	unsafe {
		...
		while j < len {
	 		*output.add(i as usize) = ((*input.add(j as usize) as UINT4)
	 			| ((*input.add((j + 1) as usize) as UINT4) << 8)
	 			| ((*input.add((j + 2) as usize) as UINT4) << 16)
	 			| ((*input.add((j + 3) as usize) as UINT4) << 24));
	 		i = i.wrapping_add(1);
	 		j = j.wrapping_add(4);
		}
	}
...
pub fn Com_BlockChecksum(buffer: *const core::ffi::c_void, length: c_int) -> u32
{
	unsafe {
