...
use core::ffi::c_int;
use super::l3_h::{SCALEFACT, GR, IS_SF_INFO};

//extern int iframe;

extern "C" {
    fn bitget(n: c_int) -> u32;
}

/*------------------------------------------------------------*/
static SLEN_TABLE: [[i32; 2]; 16] =
[
  [0, 0], [0, 1],
  [0, 2], [0, 3],
  [3, 0], [1, 1],
...
/*=============================================================*/
pub unsafe fn unpack_sf_sub_MPEG1(sf: *mut SCALEFACT,
			 grdat: &mut GR,
			 scfsi: i32,	/* bit flag */
			 gr: i32)
{
   ...
   let mut slen1: i32;
   let block_type: i32;
   let mixed_block_flag: i32;
   let scalefac_compress: i32;
   ...
   if block_type == 2
   {
	 ...
	 sfb = 0;
	 while sfb < 8 {
	    (*sf).l[sfb as usize] = bitget(slen0) as i32;
	    sfb += 1;
	 }
	 sfb = 3;
	 while sfb < 6 {
	    (*sf).s[0][sfb as usize] = bitget(slen0) as i32;
	    (*sf).s[1][sfb as usize] = bitget(slen0) as i32;
	    (*sf).s[2][sfb as usize] = bitget(slen0) as i32;
	    sfb += 1;
	 }
	 sfb = 6;
	 while sfb < 12 {
	    (*sf).s[0][sfb as usize] = bitget(slen1) as i32;
	    (*sf).s[1][sfb as usize] = bitget(slen1) as i32;
	    (*sf).s[2][sfb as usize] = bitget(slen1) as i32;
	    sfb += 1;
	 }
	 return;
      }
...
/* long blocks types 0 1 3, first granule */
   if gr == 0
   {
	 ...
	 sfb += 1;
      ...
      while sfb < 21 {
	 (*sf).l[sfb as usize] = bitget(slen1) as i32;
	 sfb += 1;
      }
      return;
   }
...
/*=============================================================*/
pub unsafe fn unpack_sf_sub_MPEG2(sf: *mut SCALEFACT,
			 grdat: &mut GR,
			 is_and_ch: i32,
			 sf_info: &mut IS_SF_INFO)
{
   ...
   let mut k: i32;
   let mut preflag: i32;
   let mut intensity_scale: i32;
   let block_type: i32;
   let mixed_block_flag: i32;
   let mut scalefac_compress: i32;
...
/* remaining short blocks */
	 ...
	 i = 0;
	 while i < nr2 {
	    (*sf).s[0][sfb as usize] = bitget(slen2) as i32;
	    (*sf).s[1][sfb as usize] = bitget(slen2) as i32;
	    (*sf).s[2][sfb as usize] = bitget(slen2) as i32;
	    i += 1;
	    sfb += 1;
	 }
      } else {
	 ...
	 i = 0;
	 while i < nr3 {
	    (*sf).s[0][sfb as usize] = bitget(slen3) as i32;
	    (*sf).s[1][sfb as usize] = bitget(slen3) as i32;
	    (*sf).s[2][sfb as usize] = bitget(slen3) as i32;
	    i += 1;
	    sfb += 1;
	 }
      } else {
	 ...
	 i = 0;
	 while i < nr4 {
	    (*sf).s[0][sfb as usize] = bitget(slen4) as i32;
	    (*sf).s[1][sfb as usize] = bitget(slen4) as i32;
	    (*sf).s[2][sfb as usize] = bitget(slen4) as i32;
	    i += 1;
	    sfb += 1;
	 }
      } else {
...
/* long blocks types 0 1 3 */
   ...
   if slen2 != 0
   {
      i = 0;
      while i < nr2 {
	 (*sf).l[sfb as usize] = bitget(slen2) as i32;
	 i += 1;
	 sfb += 1;
      }
   } else {
   ...
   if slen3 != 0
   {
      i = 0;
      while i < nr3 {
	 (*sf).l[sfb as usize] = bitget(slen3) as i32;
	 i += 1;
	 sfb += 1;
      }
   } else {
   ...
   if slen4 != 0
   {
      i = 0;
      while i < nr4 {
	 (*sf).l[sfb as usize] = bitget(slen4) as i32;
	 i += 1;
	 sfb += 1;
      }
   } else {
