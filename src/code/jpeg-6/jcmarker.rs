...
/// Emit a byte
fn emit_byte(cinfo: *mut c_void, val: c_int) {
    // Stub: requires access to cinfo->dest (jpeg_destination_mgr)
    // which would normally contain next_output_byte and free_in_buffer
}

/// Emit a marker code
fn emit_marker(cinfo: *mut c_void, mark: JPEG_MARKER) {
    /* Emit a marker code */
...
fn emit_2bytes(cinfo: *mut c_void, value: c_int) {
...
fn emit_dqt(cinfo: *mut c_void, index: c_int) -> c_int {
    /* Emit a DQT marker */
    /* Returns the precision used (0 = 8bits, 1 = 16bits) for baseline checking */
...
/// Emit a DHT marker
fn emit_dht(cinfo: *mut c_void, index: c_int, is_ac: c_int) {
    /* Emit a DHT marker */
...
/// Emit a SOF marker
fn emit_sof(cinfo: *mut c_void, code: JPEG_MARKER) {
    /* Emit a SOF marker */
...
fn emit_jfif_app0(cinfo: *mut c_void) {
    /*
...
fn emit_adobe_app14(cinfo: *mut c_void) {
    /*
...
pub fn write_any_marker(cinfo: *mut c_void, marker: c_int,
                        dataptr: *const u8, datalen: u32) {
    /* Emit an arbitrary marker with parameters */
    if datalen <= 65533u32 { /* safety check */
        emit_marker(cinfo, unsafe { core::mem::transmute::<u32, JPEG_MARKER>(marker as u32) });

        emit_2bytes(cinfo, (datalen as c_int) + 2); /* total length */
...
pub fn write_file_header(cinfo: *mut c_void) {
...
pub fn write_frame_header(cinfo: *mut c_void) {
    let mut ci: c_int;
    let mut prec: c_int;
    let mut is_baseline: c_int;
    // jpeg_component_info *compptr;
...
pub fn write_scan_header(cinfo: *mut c_void) {
    let mut i: c_int;
    // jpeg_component_info *compptr;
...
pub fn write_file_trailer(cinfo: *mut c_void) {
...
pub fn write_tables_only(cinfo: *mut c_void) {
    let mut i: c_int;
...
pub fn jinit_marker_writer(cinfo: *mut c_void) {
