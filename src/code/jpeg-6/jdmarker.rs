...
#![allow(non_snake_case, non_camel_case_types, non_upper_case_globals,
         dead_code, unused_variables, unused_assignments, unused_mut,
...
/* JPEG_INTERNALS pulls jpegint.h, jmorecfg.h, jerror.h transitively: */
...
#[repr(C)]
pub enum JPEG_MARKER {			/* JPEG marker codes */
    M_SOF0  = 0xc0,
    M_SOF1  = 0xc1,
    M_SOF2  = 0xc2,
    M_SOF3  = 0xc3,
...
/* INPUT_VARS(cinfo) -- expanded inline in each function body as three let
 * declarations (datasrc, next_input_byte, bytes_in_buffer).  Rust macro
 * hygiene prevents macro-introduced locals from being visible outside the
 * macro's expansion; inlining avoids that restriction entirely.
 */
...
macro_rules! INPUT_BYTE {
    ($cinfo:expr, $V:expr, $action:expr) => {{
        ...
        $V = GETJOCTET!(*next_input_byte) as _;
...
macro_rules! INPUT_2BYTES {
    ($cinfo:expr, $V:expr, $action:expr) => {{
        MAKE_BYTE_AVAIL!($cinfo, $action);
        $V = ((GETJOCTET!(*next_input_byte) as c_uint) << 8) as _;
        next_input_byte = next_input_byte.add(1);
        MAKE_BYTE_AVAIL!($cinfo, $action);
        $V += GETJOCTET!(*next_input_byte) as _;
        next_input_byte = next_input_byte.add(1);
...
/* Process a SOFn marker */
{
    ...
    /* INPUT_VARS(cinfo) */
    let datasrc: *mut jpeg_source_mgr = (*cinfo).src;
    let mut next_input_byte: *const JOCTET = (*datasrc).next_input_byte;
    let mut bytes_in_buffer: usize = (*datasrc).bytes_in_buffer;
...
/* Process a SOS marker */
{
    ...
    if length != (n * 2 + 6) as INT32 || n < 1 || n > MAX_COMPS_IN_SCAN {
        ERREXIT!(cinfo, JERR_BAD_LENGTH);
    }
...
pub unsafe extern "C" fn get_app0(cinfo: j_decompress_ptr) -> boolean
/* Process an APP0 marker */
{
    ...
    if length >= JFIF_LEN as INT32 {
        ...
        if b[0]==0x4A && b[1]==0x46 && b[2]==0x49 && b[3]==0x46 && b[4]==0 {
            ...
            if length != ((b[12] as INT32) * (b[13] as INT32) * (3 as INT32)) {
                TRACEMS1!(cinfo, 1, JTRC_JFIF_BADTHUMBNAILSIZE, length as c_int);
            }
        } else {
            /* Start of APP0 does not match "JFIF" */
            TRACEMS1!(cinfo, 1, JTRC_APP0, (length as c_int) + JFIF_LEN);
        }
    } else {
    ...
    if length > 0 {		/* skip any remaining data -- could be lots */
        ((*(*cinfo).src).skip_input_data)(cinfo, length as c_long);
    }
...
pub unsafe extern "C" fn get_app14(cinfo: j_decompress_ptr) -> boolean
/* Process an APP14 marker */
{
    ...
    if length >= ADOBE_LEN as INT32 {
        ...
        if b[0]==0x41 && b[1]==0x64 && b[2]==0x6F && b[3]==0x62 && b[4]==0x65 {
            ...
            TRACEMS1!(cinfo, 1, JTRC_APP14, (length as c_int) + ADOBE_LEN);
        }
    } else {
    ...
    while length > 0 {
        ...
        while i < DCTSIZE2 {
            TRACEMS8!(cinfo, 2, JTRC_QUANTVALS,
                     (*quant_ptr).quantval[(i  ) as usize], (*quant_ptr).quantval[(i+1) as usize],
                     (*quant_ptr).quantval[(i+2) as usize], (*quant_ptr).quantval[(i+3) as usize],
                     (*quant_ptr).quantval[(i+4) as usize], (*quant_ptr).quantval[(i+5) as usize],
                     (*quant_ptr).quantval[(i+6) as usize], (*quant_ptr).quantval[(i+7) as usize]);
            i += 8;
        }
...
pub unsafe extern "C" fn skip_variable(cinfo: j_decompress_ptr) -> boolean
/* Skip over an unknown or uninteresting variable-length marker */
{
    ...
    INPUT_SYNC!(cinfo);		/* do before skip_input_data */
    ((*(*cinfo).src).skip_input_data)(cinfo, (length as c_long) - 2);
 ...
 */
{
    ...
    if c != 0xFF || c2 != (JPEG_MARKER::M_SOI as c_int) {
...
pub unsafe extern "C" fn read_markers(cinfo: j_decompress_ptr) -> c_int
{
    ...
    loop {
        ...
        match (*cinfo).unread_marker {
            ...
            0xc0 |		/* M_SOF0: Baseline */
            0xc1 => {		/* M_SOF1: Extended sequential, Huffman */
            ...
            0xc3 |		/* M_SOF3: Lossless, Huffman */
            0xc5 |		/* M_SOF5: Differential sequential, Huffman */
            0xc6 |		/* M_SOF6: Differential progressive, Huffman */
            0xc7 |		/* M_SOF7: Differential lossless, Huffman */
            0xc8 |		/* M_JPG: Reserved for JPEG extensions */
            0xcb |		/* M_SOF11: Lossless, arithmetic */
            0xcd |		/* M_SOF13: Differential sequential, arithmetic */
            0xce |		/* M_SOF14: Differential progressive, arithmetic */
            0xcf => {		/* M_SOF15: Differential lossless, arithmetic */
            ...
            0xee |		/* M_APP14 */
            0xef => {		/* M_APP15 */
                if !((*(*cinfo).marker).process_APPn[
                        ((*cinfo).unread_marker - (JPEG_MARKER::M_APP0 as c_int)) as usize])
                    (cinfo) {
                    return JPEG_SUSPENDED;
                }
            }
            ...
            0xd0 |		/* M_RST0: these are all parameterless */
            0xd1 |		/* M_RST1 */
            0xd2 |		/* M_RST2 */
            0xd3 |		/* M_RST3 */
            0xd4 |		/* M_RST4 */
...
pub unsafe extern "C" fn read_restart_marker(cinfo: j_decompress_ptr) -> boolean
{
    ...
    if (*cinfo).unread_marker ==
        ((JPEG_MARKER::M_RST0 as c_int) + (*(*cinfo).marker).next_restart_num) {
...
pub unsafe fn jpeg_resync_to_restart(cinfo: j_decompress_ptr, desired: c_int) -> boolean
{
    ...
    loop {
        if marker < (JPEG_MARKER::M_SOF0 as c_int) {
            action = 2;		/* invalid marker */
        } else if marker < (JPEG_MARKER::M_RST0 as c_int) || marker > (JPEG_MARKER::M_RST7 as c_int) {
            action = 3;		/* valid non-restart marker */
        } else {
            if marker == ((JPEG_MARKER::M_RST0 as c_int) + ((desired+1) & 7)) ||
                marker == ((JPEG_MARKER::M_RST0 as c_int) + ((desired+2) & 7)) {
                action = 3;		/* one of the next two expected restarts */
            } else if marker == ((JPEG_MARKER::M_RST0 as c_int) + ((desired-1) & 7)) ||
                      marker == ((JPEG_MARKER::M_RST0 as c_int) + ((desired-2) & 7)) {
...
pub unsafe extern "C" fn reset_marker_reader(cinfo: j_decompress_ptr)
{
