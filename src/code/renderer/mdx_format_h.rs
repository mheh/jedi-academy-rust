...
use core::ffi::{c_int, c_float, c_uint, c_uchar, c_char};

// Stub declarations for external symbols referenced by this header
pub const MAX_QPATH: usize = 64;

pub type vec3_t = [c_float; 3];
pub type vec2_t = [c_float; 2];
...
//
pub const G2BONEFLAG_ALWAYSXFORM: c_uint = 0x00000001;

// same thing but for surfaces...  (Carcass will only generate 1st 2 flags, others are ingame
//
pub const G2SURFACEFLAG_ISBOLT: c_uint = 0x00000001;
pub const G2SURFACEFLAG_OFF: c_uint = 0x00000002;	// saves strcmp()ing for "_off" in surface names
pub const G2SURFACEFLAG_SPARE0: c_uint = 0x00000004;	// future-expansion fields, saves invalidating models if we add more
pub const G2SURFACEFLAG_SPARE1: c_uint = 0x00000008;	//
pub const G2SURFACEFLAG_SPARE2: c_uint = 0x00000010;	//
pub const G2SURFACEFLAG_SPARE3: c_uint = 0x00000020;	//
pub const G2SURFACEFLAG_SPARE4: c_uint = 0x00000040;	//
pub const G2SURFACEFLAG_SPARE5: c_uint = 0x00000080;	//
//
pub const G2SURFACEFLAG_NODESCENDANTS: c_uint = 0x00000100;	// ingame-stuff, never generated by Carcass....
pub const G2SURFACEFLAG_GENERATED: c_uint = 0x00000200;	//
...
pub const iMAX_G2_BONEWEIGHTS_PER_VERT: usize = 4;	// can't just be blindly increased, affects cache size etc

pub const iG2_BONEWEIGHT_TOPBITS_SHIFT: c_int = (5 * 4) - 8;	// 8 bits because of 8 in the BoneWeight[] array entry
pub const iG2_BONEWEIGHT_TOPBITS_AND: c_uint = 0x300;	// 2 bits, giving 10 total, or 10 bits, for 1023/1024 above
...
pub struct mdxaCompQuatBone_t {
	...
	// I'm defining this '<' operator so this struct can be used as an STL <map> key...
	//
	// (Note: operator< not translated; Rust doesn't have C++ operator overloading)
}
		...
		// {
				...
				// {
						...
						pub struct mdxmVertex_t {
							// NOTE: _XBOX variant not translated; using standard variant
							// #[cfg(target_os = "xbox")] public fields:
							// pub normal: c_uint,
							...
							// packed int...
							pub uiNmWeightsAndBoneIndexes: c_uint,	// 32 bits.  format:
...
{
	unsafe {
		let iNumWeights = ((*pVert).uiNmWeightsAndBoneIndexes >> 30) + 1;	// 1..4 count

		return iNumWeights as c_int;
	}
}
...
{
	unsafe {
		let iBoneIndex = ((*pVert).uiNmWeightsAndBoneIndexes >> (iG2_BITS_PER_BONEREF * iWeightNum)) & ((1 << iG2_BITS_PER_BONEREF) - 1);

		return iBoneIndex as c_int;
	}
}
...
pub fn G2_GetVertBoneWeight(pVert: *const mdxmVertex_t, iWeightNum: c_int, fTotalWeight: &mut f32, iNumWeights: c_int) -> f32
{
	unsafe {
		...
		{
			let mut iTemp = (*pVert).BoneWeightings[iWeightNum as usize] as c_uint;
...
pub struct mdxaHeader_t {
	...
	pub fScale: c_float,				// will be zero if build before this field was defined, else scale it was built with
