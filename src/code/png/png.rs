//! Mechanical port of `code/png/png.cpp`.
...
// Minimal z_stream structure for FFI compatibility
#[repr(C)]
pub struct z_stream {
    pub next_in: *mut byte,
    pub avail_in: ulong,
    pub total_in: ulong,
...
unsafe extern "C" {
    ...
    fn Z_Malloc(iSize: c_int, eTag: c_int, bZeroit: c_int) -> *mut c_void;
    fn Z_Free(ptr: *mut c_void);

    fn FS_FOpenFileWrite(qpath: *const c_char) -> c_int;
    ...
    fn crc32(crc: ulong, buf: *const byte, len: ulong) -> ulong;
    fn deflateInit(strm: *mut z_stream, level: ELevel, noWrap: c_int) -> EStatus;
    fn deflate(strm: *mut z_stream, flush: EFlush) -> EStatus;
    fn deflateEnd(strm: *mut z_stream) -> EStatus;
    fn inflateInit(strm: *mut z_stream, flush: EFlush, noWrap: c_int) -> EStatus;
    fn inflate(z: *mut z_stream) -> EStatus;
    fn inflateEnd(strm: *mut z_stream) -> EStatus;
}
...
pub unsafe extern "C" fn PNG_Load(
    ...
    workspace = Z_Malloc(datasize as c_int, TAG_TEMP_PNG, 0) as *mut byte;
    work = workspace;
    totallength = 0;
...
pub unsafe extern "C" fn PNG_Save(
    ...
    work = Z_Malloc(maxsize, TAG_TEMP_PNG, 1) as *mut byte; // fixme: optimise to qfalse sometime - ok?
...
pub unsafe extern "C" fn PNG_ConvertTo32(image: *mut png_image_t) {
    ...
    temp = Z_Malloc(((*image).width * (*image).height * 4) as c_int, TAG_TEMP_PNG, 1)
        as *mut byte;
    old = (*image).data;
    old2 = old;
    (*image).data = temp;
