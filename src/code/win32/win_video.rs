...
extern "C" {
    pub fn SNDDMA_GetDSHandle() -> c_uint;
    pub static mut s_soundStarted: c_int; // if 0, game is running in silent mode
}

// ret qtrue if success...
//
pub fn VIDEO_GetDims(piWidth: *mut c_int, piHeight: *mut c_int) -> c_int {
    /*
    if (hBink)
    {
        if (piWidth && piHeight)
...
//
pub fn VIDEO_NextFrameReady() -> c_int {
...
pub fn VIDEO_Pause(bPaused: c_int) {
    /*
    if (hBink)
    {
...
pub fn VIDEO_NextFrame(pbDestBuffer: *mut u8, iBytesPerLine: c_int, bFrameSkipped: *mut c_int) -> c_int {
    /*
    if (hBink)
    {
...
//}
    ...
    */
    0 // qfalse	will only get here if some sort of error in Q3 logic (which does happen) so that this is called
      //	at the wrong time.
}
...
//
pub fn VIDEO_Open(psPathlessBaseName: *mut c_char, qbInGame: c_int, qbTestOpenOnly: c_int, iLanguageNumber: c_int) -> c_int {
    /*
