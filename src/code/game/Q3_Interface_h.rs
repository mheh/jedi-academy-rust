...
// Local stubs for C++ dependencies (needed for structural coherence of this header translation)
pub struct string;
...
pub struct map<K, V> {
    _marker: (std::marker::PhantomData<K>, std::marker::PhantomData<V>),
}
...
// Forward declaration stub for IGameInterface (from IcarusInterface.h)
pub struct IGameInterface;

impl IGameInterface {
    pub unsafe fn GetGame() -> *mut IGameInterface {
        // Stub - actual implementation is in engine
        std::ptr::null_mut()
    }
}

// Enum setType_e (typedef'd as setType_t)
#[repr(C)]
...
pub enum setType_t {
    ...
    */
    ...
    //# #eol (no actual variant here, just marking end of enum)
}

// Enum playType_e (typedef'd as playType_t)
// this enum isn't used directly by the game, it's mainly for BehavEd to scan for...
#[repr(C)]
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum playType_t {
    // Types of file to play
...
pub const Q3_TIME_SCALE: c_int = 1; // MILLISECONDS

pub static mut cinematicSkipScript: [c_char; 64] = [0; 64];

// General
extern "C" {
    pub fn Q3_TaskIDClear(taskID: *mut c_int);
    pub fn Q3_TaskIDPending(ent: *mut gentity_t, taskType: taskID_t) -> c_int;
    pub fn Q3_TaskIDComplete(ent: *mut gentity_t, taskType: taskID_t);
    pub fn Q3_DPrintf(fmt: *const c_char, ...);

    // Not referenced directly as script function - all are called through Q3_Set
    pub fn Q3_SetAnimBoth(entID: c_int, anim_name: *const c_char);
    pub fn Q3_SetVelocity(entID: c_int, angles: *const [f32; 3]);
}
...
pub const TKERR_NONE: c_int = 0;
pub const TKERR_UNKNOWN: c_int = 1;
pub const TKERR_BUFFERCREATE: c_int = 2;
pub const TKERR_UNRECOGNIZEDSYMBOL: c_int = 3;
pub const TKERR_DUPLICATESYMBOL: c_int = 4;
pub const TKERR_STRINGLENGTHEXCEEDED: c_int = 5;
pub const TKERR_IDENTIFIERLENGTHEXCEEDED: c_int = 6;
pub const TKERR_EXPECTED_INTEGER: c_int = 7;
pub const TKERR_EXPECTED_IDENTIFIER: c_int = 8;
pub const TKERR_EXPECTED_STRING: c_int = 9;
pub const TKERR_EXPECTED_CHAR: c_int = 10;
pub const TKERR_EXPECTED_FLOAT: c_int = 11;
pub const TKERR_UNEXPECTED_TOKEN: c_int = 12;
pub const TKERR_INVALID_DIRECTIVE: c_int = 13;
pub const TKERR_INCLUDE_FILE_NOTFOUND: c_int = 14;
pub const TKERR_UNMATCHED_DIRECTIVE: c_int = 15;
pub const TKERR_USERERROR: c_int = 16;
...
pub const TK_INTEGER: c_int = TK_INT;
pub const TK_FLOAT: c_int = 6;
pub const TK_IDENTIFIER: c_int = 7;
pub const TK_USERDEF: c_int = 8;

// Token defines
pub const TK_BLOCK_START: c_int = TK_USERDEF;
pub const TK_BLOCK_END: c_int = TK_USERDEF + 1;
pub const TK_VECTOR_START: c_int = TK_USERDEF + 2;
pub const TK_VECTOR_END: c_int = TK_USERDEF + 3;
pub const TK_OPEN_PARENTHESIS: c_int = TK_USERDEF + 4;
pub const TK_CLOSED_PARENTHESIS: c_int = TK_USERDEF + 5;
pub const TK_VECTOR: c_int = TK_USERDEF + 6;
pub const TK_GREATER_THAN: c_int = TK_USERDEF + 7;
pub const TK_LESS_THAN: c_int = TK_USERDEF + 8;
pub const TK_EQUALS: c_int = TK_USERDEF + 9;
pub const TK_NOT: c_int = TK_USERDEF + 10;

pub const NUM_USER_TOKENS: c_int = TK_USERDEF + 11;
...
// Type defines
pub const TYPE_WAIT_COMPLETE: c_int = NUM_IDS; // Wait types
pub const TYPE_WAIT_TRIGGERED: c_int = NUM_IDS + 1;

pub const TYPE_ANGLES: c_int = NUM_IDS + 2; // Set types
pub const TYPE_ORIGIN: c_int = NUM_IDS + 3;

pub const TYPE_INSERT: c_int = NUM_IDS + 4; // Affect types
pub const TYPE_FLUSH: c_int = NUM_IDS + 5;

pub const TYPE_PAN: c_int = NUM_IDS + 6; // Camera types
...
pub const TYPE_VARIABLE: c_int = NUM_IDS + 18; // Variable type

pub const TYPE_EOF: c_int = NUM_IDS + 19;

pub const ID_CONTINUE: c_int = NUM_IDS + 20;
pub const ID_BREAK: c_int = NUM_IDS + 21;
pub const ID_ACTIVATE: c_int = NUM_IDS + 22;
pub const ID_DEACTIVATE: c_int = NUM_IDS + 23;
pub const ID_WHILE: c_int = NUM_IDS + 24;
pub const ID_WAITUNTIL: c_int = NUM_IDS + 25;
pub const ID_WAITAFFECT: c_int = NUM_IDS + 26;
pub const ID_TASKCOMPLETED: c_int = NUM_IDS + 27;
pub const ID_SIGNALCOUNT: c_int = NUM_IDS + 28;
pub const TK_GE: c_int = NUM_IDS + 29;
pub const TK_LE: c_int = NUM_IDS + 30;

pub const NUM_TYPES: c_int = NUM_IDS + 31;
...
pub const Q3_SCRIPT_DIR: &[u8] = b"scripts";

pub const MAX_FILENAME_LENGTH: usize = 256;

pub const IBI_EXT: &[u8] = b".IBI"; // (I)nterpreted (B)lock (I)nstructions
...
pub type scriptlist_t = map<string, *mut pscript_t>;
pub type entitylist_t = map<string, c_int>;

// STL map type definitions for the variable containers.
pub type varString_m = map<string, string>;
pub type varFloat_m = map<string, f32>;

// The Quake 3 Game Interface Class for Quake3 and Icarus to use.
// Created: 10/08/02 by Aurelio Reis.
pub struct CQuake3GameInterface {
    // Base class would be IGameInterface in C++, represented as an inherited structure
    // Marking this as a struct that logically "extends" IGameInterface

    // Private members (represented with pub visibility for Rust struct fields)
    // A list of script buffers (to ensure we cache a script only once).
    pub m_ScriptList: scriptlist_t,

    // A list of Game Elements/Objects.
    pub m_EntityList: entitylist_t,

    // Variable stuff.
    pub m_varStrings: varString_m,
    pub m_varFloats: varFloat_m,
    pub m_varVectors: varString_m,
    pub m_numVariables: c_int,

    pub m_entFilter: c_int,
}

// Private helper functions (declared but implementation is in the C++ .cpp file):
...
// Static Singleton Instance (would be stored in actual implementation):
// static CQuake3GameInterface *m_pInstance;

// Public methods (variable enums):
// enum { VTYPE_NONE = 0, VTYPE_FLOAT, VTYPE_STRING, VTYPE_VECTOR, MAX_VARIABLES = 32 };

// Public register enums:
// enum { SCRIPT_COULDNOTREGISTER = 0, SCRIPT_REGISTERED, SCRIPT_ALREADYREGISTERED };

// Constructor and destructor (declared in .cpp):
// CQuake3GameInterface();
// ~CQuake3GameInterface();

// Public method declarations (implementations in .cpp):
...
// Accessor function for accessing Quake 3 Interface specific functions.
#[inline]
pub fn Quake3Game() -> *mut CQuake3GameInterface {
    unsafe { IGameInterface::GetGame() as *mut CQuake3GameInterface }
}
