...
use crate::code::RMG::RM_Instance_BSP_h::*;
use crate::code::RMG::RM_Instance_Random_h::*;
use crate::code::RMG::RM_Instance_Group_h::*;
use crate::code::RMG::RM_Instance_Void_h::*;
// Porting note: CRMInstanceFile is defined in the paired header; import from there per triage caution.
use crate::code::RMG::RM_InstanceFile_h::*;
...
impl CRMInstanceFile {
    ...
    pub unsafe fn new() -> Self {
        // C++ default-constructs mParser; zero-init is the faithful approximation for a blind port.
        let mut s: CRMInstanceFile = core::mem::zeroed();
    ...
    pub unsafe fn CreateInstance(&mut self, name: *const c_char) -> *mut CRMInstance {
        ...
        while !group.is_null() {
            ...
            if stricmp((*group).GetName(), b"bsp\0".as_ptr() as *const c_char) == 0 {
                instance =
                    Box::into_raw(Box::new(CRMBSPInstance::new(group, self))) as *mut CRMInstance;
...
//			instance = new CRMNPCInstance ( group, *this );
                ...
                instance = Box::into_raw(Box::new(CRMGroupInstance::new(group, self)))
                    as *mut CRMInstance;
            } else if stricmp((*group).GetName(), b"random\0".as_ptr() as *const c_char) == 0 {
                instance = Box::into_raw(Box::new(CRMRandomInstance::new(group, self)))
                    as *mut CRMInstance;
            } else if stricmp((*group).GetName(), b"void\0".as_ptr() as *const c_char) == 0 {
                instance = Box::into_raw(Box::new(CRMVoidInstance::new(group, self)))
                    as *mut CRMInstance;
            } else {
                group = (*group).GetNext();
                continue;
