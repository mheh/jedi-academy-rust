//! Mechanical port of `codemp/RMG/RM_Terrain.cpp`.
...
impl CRandomModel {
    ...
    /// Stub for `const char* CRandomModel::GetModel()`.
    pub fn GetModel(&self) -> *const c_char {
        core::ptr::null()
    }
...
/// Stub for unported `TCGMiscEnt` (client.h).
/// Misc entity data structure passed to client game.
#[repr(C)]
pub struct TCGMiscEnt {
    pub mModel: [c_char; 256],
    pub mOrigin: [f32; 3],
    pub mAngles: [f32; 3],
    pub mScale: [f32; 3],
}

impl Default for TCGMiscEnt {
    fn default() -> Self {
        TCGMiscEnt {
            mModel: [0; 256],
            mOrigin: [0.0; 3],
...
/// Stub for unported `TCGTrace` (client.h).
/// Trace/collision query structure.
#[repr(C)]
pub struct TCGTrace {
    pub mStart: [f32; 3],
    pub mEnd: [f32; 3],
    ...
    pub mMask: c_int,
    pub mResult: TraceResult,
}

/// Stub for unported `TraceResult` (collision).
#[repr(C)]
...
extern "C" {
    ...
    /// C standard library function to compare strings (case-sensitive).
    ...
    fn memcpy(dest: *mut c_void, src: *const c_void, n: core::ffi::c_size_t) -> *mut c_void;
    ...
    /// Quake engine global VM call function.
    fn VM_Call(vm: *mut c_void, cmd: c_int, ...);

    /// Quake engine global cgvm.
    static mut cgvm: *mut c_void;

    /// Quake engine global cl (client).
    static mut cl: Client;

    /// Quake engine global cmg (collision manager global).
    static mut cmg: CMGlobal;
}

/// Stub for unported `Client` structure.
pub struct Client {
...
const MAX_RANDOM_MODELS: usize = 64;
const HEIGHT_RESOLUTION: c_int = 256;
const TAG_TERRAIN: c_int = 32;

// VM call commands
const CG_TRACE: c_int = 1;
const CG_MISC_ENT: c_int = 2;
...
// Collision mask
const MASK_PLAYERSOLID: c_int = 0x0001;
...
impl CCGHeightDetails {
    ...
    pub fn AddModel(&mut self, hd: &CRandomModel) {
        if self.mNumModels < MAX_RANDOM_MODELS {
            self.mTotalFrequency += hd.GetFrequency();
            self.mModels[self.mNumModels] = *hd;
            self.mNumModels += 1;
        }
    }

    /// Get the number of models at this height level.
    pub fn GetNumModels(&self) -> usize {
        self.mNumModels
    }

    /// Get the total frequency (used for weighted random selection).
    pub fn GetTotalFrequency(&self) -> f32 {
        self.mTotalFrequency
    }

    /// Get the average frequency of models at this height.
    pub fn GetAverageFrequency(&self) -> f32 {
        if self.mNumModels > 0 {
            self.mTotalFrequency / self.mNumModels as f32
        } else {
            0.0
        }
    }
    ...
    pub fn GetRandomModel(&self, land: *mut CCMLandScape) -> *mut CRandomModel {
        let land_ref = unsafe { &*land };
        let mut seek = land_ref.irand(0, self.mTotalFrequency as c_int);
        for i in 0..self.mNumModels {
            seek -= self.mModels[i].GetFrequency() as c_int;
            if seek <= 0 {
                return unsafe { (self.mModels.as_ptr() as *mut CRandomModel).add(i) };
            }
        }
        unsafe {
            #[cfg_attr(feature = "safe", allow(unreachable_code))]
            {
...
impl CRMLandScape {
    ...
    pub fn SetCommon(&mut self, land: *mut CCMLandScape) {
        self.common = land;
    }
    ...
    pub fn AddModel(&mut self, height: c_int, mut maxheight: c_int, hd: &CRandomModel) {
        ...
        let mut i = height;
        while i < maxheight && !hd.GetModel().is_null() {
                        ...
                        while !model.is_null() {
                            if stricmp((*model).GetName(), b"model\0".as_ptr() as *const c_char) == 0 {
                                ...
                                while !pair.is_null() {
                                    ...
                                    } else if stricmp((*pair).GetName(), b"minscale\0".as_ptr() as *const c_char)
                                        == 0
                                    {
                                        hd.SetMinScale(atof((*pair).GetTopValue()) as f32);
                                    } else if stricmp((*pair).GetName(), b"maxscale\0".as_ptr() as *const c_char)
                                        == 0
                                    {
    ...
    pub fn CreateRandomDensityMap(&mut self, density: *mut u8, width: c_int, height: c_int, _seed: c_int) {
        let x: c_int;
        let y: c_int;
        let mut count: c_int;
        let area: *mut CArea;
        let derxelSize: [f32; 3];
    ...
    pub fn LoadDensityMap(&mut self, td: *const c_char) {
        ...
        let imageData: *mut u8;
        let iWidth: c_int;
        let iHeight: c_int;
        let seed: c_int;
        let ptr: *mut *const c_char = core::ptr::null_mut();

        unsafe {
            // Fill in with default values
            self.mDensityMap = Z_Malloc((*self.common).GetBlockCount(), TAG_TERRAIN) as *mut u8;
            memset(self.mDensityMap as *mut c_void, 128, (*self.common).GetBlockCount() as core::ffi::c_size_t);
            ...
            if strlen(densityMap.as_ptr()) > 0 {
                ...
                {
                    let mut ptrImageData: *mut u8 = core::ptr::null_mut();
                    let mut ptrIWidth: c_int = 0;
                    let mut ptrIHeight: c_int = 0;
                    R_LoadDataImage(densityMap.as_ptr(), &mut ptrImageData, &mut ptrIWidth, &mut ptrIHeight);
                    if !ptrImageData.is_null() {
                        if !strstr(densityMap.as_ptr(), b"density_\0".as_ptr() as *const c_char).is_null() {
                            let mut ptr_seed: *mut c_char = core::ptr::null_mut();
                            seed = strtoul(
                                Info_ValueForKey(td, b"seed\0".as_ptr() as *const c_char),
                                &mut (ptr_seed as *mut *const c_char),
                                10,
                            ) as c_int;
                            self.CreateRandomDensityMap(ptrImageData, ptrIWidth, ptrIHeight, seed);
                        }
                        R_Resample(
                            ptrImageData,
                            ptrIWidth,
                        ...
                        Z_Free(ptrImageData as *mut c_void);
                    }
                }
            }
        }
    ...
    pub fn Sprinkle(&mut self, patch: *mut CCMPatch, hd: *mut CCGHeightDetails, level: c_int) {
        unsafe {
            #[cfg(not(feature = "dedicated"))]
            {
                ...
                let data = cl.mSharedMemory as *mut TCGMiscEnt;
                let td = cl.mSharedMemory as *mut TCGTrace;

                px = (*patch).GetHeightMapX() / (*self.common).GetTerxels();
                py = (*patch).GetHeightMapY() / (*self.common).GetTerxels();
                // Get a number -5.3f to 5.3f
                density = ((*self.mDensityMap.add((px + ((*self.common).GetBlockWidth() * py)) as usize) as f32 - 128.0) / 24.0);
                // ..and multiply that into the count
                count = Round(
                    (*self.common).GetPatchScalarSize()
                        * (*hd).GetAverageFrequency()
                        * powf(2.0, density)
                        * 0.001,
                );
                ...
                while i < count {
                    if (*self.common).irand(0, 10) == 0 {
                        ...
                        memset(td as *mut c_void, 0, core::mem::size_of::<TCGTrace>());
                        VectorCopy(origin, &mut (*td).mStart);
                        VectorCopy(bounds[0], &mut (*td).mMins);
                        VectorCopy(bounds[1], &mut (*td).mMaxs);
                        VectorCopy(origin, &mut (*td).mEnd);
                        (*td).mSkipNumber = -1;
                        ...
                        strcpy_s((*data).mModel.as_mut_ptr(), (*data).mModel.len(), (*rm).GetModelName());
                        VectorCopy(origin, &mut (*data).mOrigin);
                        VectorCopy(angles, &mut (*data).mAngles);
                        VectorCopy(scale, &mut (*data).mScale);
...
extern "C" fn CG_Decrease(work: *mut u8, _lerp: f32, _info: *mut c_int) {
    unsafe {
        ...
        *work = Com_Clampi(1, 255, val) as u8;
    }
}
...
pub fn RM_CreateRandomModels(terrainId: c_int, terrainInfo: *const c_char) {
    unsafe {
        let landscape = Box::into_raw(Box::new(CRMLandScape::new()));
...
#[inline]
fn strcpy_s(dest: *mut c_char, dest_size: usize, src: *const c_char) {
    unsafe {
        if dest.is_null() || src.is_null() {
            return;
        }
        let src_len = strlen(src);
        let copy_len = if src_len >= dest_size { dest_size - 1 } else { src_len };
        memcpy(dest as *mut c_void, src as *const c_void, copy_len);
        *dest.add(copy_len) = 0;
    }
}
