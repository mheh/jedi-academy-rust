//! Mechanical port of `codemp/cgame/fx_rocketlauncher.c`.
//!
...
/// Full struct not needed for fx_rocketlauncher.c; only lerpOrigin and currentState.pos.trDelta required.
#[repr(C)]
...
pub struct weaponInfo_s {
    // Fields not needed for fx_rocketlauncher.c
}
...
pub struct cgEffects_t {
    pub _pad0: [u8; 5732], // Padding to reach rocket effects
...
pub struct cgs_t {
    ...
    pub _media: [u8; 5376], // cgMedia_t, padding to reach effects
...
---------------------------
*/
/// Projects the rocket projectile effect along its trajectory.
///
/// # Safety
/// `cent` must be valid; `weapon` may be unused.
pub unsafe extern "C" fn FX_RocketProjectileThink(cent: *const centity_t, weapon: *const weaponInfo_s) {
...
pub unsafe extern "C" fn FX_RocketHitWall(origin: *const vec3_t, normal: *const vec3_t) {
    trap_FX_PlayEffectID(
        (*cgs).effects.rocketExplosionEffect,
        origin,
        normal,
        -1,
        -1,
...
---------------------------
*/
/// Plays the rocket flesh impact effect.
///
/// # Safety
...
---------------------------
*/
/// Projects the rocket alt-fire projectile effect along its trajectory.
///
/// # Safety
/// `cent` must be valid; `weapon` may be unused.
pub unsafe extern "C" fn FX_RocketAltProjectileThink(cent: *const centity_t, weapon: *const weaponInfo_s) {
    let mut forward: vec3_t = [0.0; 3];
