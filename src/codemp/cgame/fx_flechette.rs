//! Mechanical port of `codemp/cgame/fx_flechette.c`.
//!
//! Golan Arms Flechette weapon effects: projectile, impact, and detonation visual effects.
...
/// Full struct not needed for fx_flechette.c; only lerpOrigin and currentState.pos.trDelta required.
#[repr(C)]
...
pub struct weaponInfo_s {
    // Fields not needed for fx_flechette.c
}
...
-------------------------
*/
/// Projects the Flechette projectile effect along its trajectory.
///
/// # Safety
/// `cent` must be valid; `weapon` may be unused.
pub unsafe extern "C" fn FX_FlechetteProjectileThink(cent: *const centity_t, weapon: *const weaponInfo_s) {
...
-------------------------
*/
/// Plays the Flechette wall impact effect.
///
/// # Safety
/// `origin` and `normal` must be valid vec3_t pointers.
pub unsafe extern "C" fn FX_FlechetteWeaponHitWall(origin: *const vec3_t, normal: *const vec3_t) {
    trap_FX_PlayEffectID(
        (*cgs).effects.flechetteWallImpactEffect,
        origin,
        normal,
        -1,
        -1,
...
-------------------------
*/
/// Plays the Flechette flesh impact effect.
///
/// # Safety
...
-------------------------
*/
/// Projects the Flechette alt-fire projectile effect along its trajectory.
///
/// # Safety
/// `cent` must be valid; `weapon` may be unused.
pub unsafe extern "C" fn FX_FlechetteAltProjectileThink(cent: *const centity_t, weapon: *const weaponInfo_s) {
    let mut forward: vec3_t = [0.0; 3];
