...
pub const CLIP_EPSILON: f32 = 0.1f32;
pub const MAX_MAP_BOUNDS: i32 = 65535;

// On_epsilon can be defined in makefile, default to 0.1f
#[allow(non_upper_case_globals)]
pub const ON_EPSILON: f32 = 0.1f32;
...
pub struct winding_t {
    pub numpoints: c_int,
    pub p: [vec3_t; 4], // variable sized, but we reserve up to capacity
}
...
extern "C" {
    pub fn Z_Malloc(size: usize, tag: c_int, clear: c_int) -> *mut c_void;
    ...
    pub fn VectorNormalize2(v: *const vec3_t, out: *mut vec3_t) -> vec_t;
    pub fn VectorLength(v: *const vec3_t) -> vec_t;
    pub fn sqrt(x: c_float) -> c_float;
}

// Utility stubs for vector operations (inline macros from C)
#[inline]
...
#[inline]
pub fn VectorCopy(a: *const vec3_t, b: *mut vec3_t) {
    unsafe {
        (*b)[0] = (*a)[0];
        (*b)[1] = (*a)[1];
        (*b)[2] = (*a)[2];
...
#[inline]
pub fn VectorScale(v: *const vec3_t, s: f32, o: *mut vec3_t) {
    unsafe {
        (*o)[0] = (*v)[0] * s;
        (*o)[1] = (*v)[1] * s;
        (*o)[2] = (*v)[2] * s;
...
#[inline]
pub fn VectorMA(v: *const vec3_t, s: f32, b: *const vec3_t, o: *mut vec3_t) {
    unsafe {
        (*o)[0] = (*v)[0] + (*b)[0] * s;
        (*o)[1] = (*v)[1] + (*b)[1] * s;
        (*o)[2] = (*v)[2] + (*b)[2] * s;
...
pub fn AllocWinding(points: c_int) -> *mut winding_t {
    ...
    unsafe {
        ...
        s = std::mem::size_of::<vec_t>() * 3 * points as usize + std::mem::size_of::<c_int>();
...
pub fn FreeWinding(w: *mut winding_t) {
    unsafe {
        ...
        if *w_ptr == 0xdeaddead {
            Com_Error(
                1, // ERR_DROP
                "FreeWinding: freed a freed winding\0".as_ptr() as *const c_char,
            );
        }
        *w_ptr = 0xdeaddead;
...
pub fn RemoveColinearPoints(w: *mut winding_t) {
    unsafe {
        ...
        let mut nump: c_int;
        let mut p: [vec3_t; 64] = [[0.0; 3]; 64]; // MAX_POINTS_ON_WINDING = 64
        ...
        Com_Memcpy(
            ...
            (nump as usize) * std::mem::size_of::<vec3_t>(),
        );
    }
}
...
pub fn WindingArea(w: *mut winding_t) -> vec_t {
    unsafe {
        ...
        while i < (*w).numpoints {
            ...
            CrossProduct(addr_of!(d1), addr_of_mut!(d2), addr_of_mut!(cross));
...
pub fn WindingBounds(w: *mut winding_t, mins: *mut vec3_t, maxs: *mut vec3_t) {
    unsafe {
        ...
        (*mins)[0] = MAX_MAP_BOUNDS as f32;
        (*mins)[1] = MAX_MAP_BOUNDS as f32;
        (*mins)[2] = MAX_MAP_BOUNDS as f32;
        (*maxs)[0] = -(MAX_MAP_BOUNDS as f32);
        (*maxs)[1] = -(MAX_MAP_BOUNDS as f32);
        (*maxs)[2] = -(MAX_MAP_BOUNDS as f32);
...
pub fn BaseWindingForPlane(normal: *mut vec3_t, dist: vec_t) -> *mut winding_t {
    unsafe {
        ...
        if x == -1 {
            Com_Error(
                1, // ERR_DROP
                "BaseWindingForPlane: no axis found\0".as_ptr() as *const c_char,
            );
        }
        ...
        VectorScale(addr_of!(vup), MAX_MAP_BOUNDS as f32, addr_of_mut!(vup));
        VectorScale(addr_of!(vright), MAX_MAP_BOUNDS as f32, addr_of_mut!(vright));
...
pub fn CopyWinding(w: *mut winding_t) -> *mut winding_t {
    unsafe {
        ...
        size = std::mem::size_of::<winding_t>() as c_int + (*w).numpoints * std::mem::size_of::<vec3_t>() as c_int;
        Com_Memcpy(
            c as *mut c_void,
            w as *const c_void,
            size as usize,
    ...
    unsafe {
        ...
        *back = AllocWinding(maxpts);
        ...
        if (*f).numpoints > maxpts || (*b).numpoints > maxpts {
            Com_Error(
                1, // ERR_DROP
                "ClipWinding: points exceeded estimate\0".as_ptr() as *const c_char,
            );
        }
        if (*f).numpoints > MAX_POINTS_ON_WINDING || (*b).numpoints > MAX_POINTS_ON_WINDING {
            Com_Error(
                1, // ERR_DROP
                "ClipWinding: MAX_POINTS_ON_WINDING\0".as_ptr() as *const c_char,
            );
        }
    }
}
...
pub fn CheckWinding(w: *mut winding_t) {
    unsafe {
        ...
        if (*w).numpoints < 3 {
            Com_Error(
                1, // ERR_DROP
                "CheckWinding: %i points\0".as_ptr() as *const c_char,
                (*w).numpoints,
            );
        }
        ...
        if area < 1.0 {
            Com_Error(
                1, // ERR_DROP
                "CheckWinding: %f area\0".as_ptr() as *const c_char,
                area,
            );
        }
        ...
        while i < (*w).numpoints {
            ...
            j = 0;
            while j < 3 {
                if (*p1)[j as usize] > (MAX_MAP_BOUNDS as f32) || (*p1)[j as usize] < (-(MAX_MAP_BOUNDS as f32)) {
                    Com_Error(
                        1, // ERR_DROP
                        "CheckFace: BUGUS_RANGE: %f\0".as_ptr() as *const c_char,
                        (*p1)[j as usize],
                    );
                }
                j += 1;
            ...
            if d < -ON_EPSILON || d > ON_EPSILON {
                Com_Error(
                    1, // ERR_DROP
                    "CheckWinding: point off plane\0".as_ptr() as *const c_char,
                );
            }
            ...
            if VectorLength(addr_of!(dir)) < ON_EPSILON {
                Com_Error(
                    1, // ERR_DROP
                    "CheckWinding: degenerate edge\0".as_ptr() as *const c_char,
                );
            }
            ...
            while j < (*w).numpoints {
                ...
                if d > edgedist {
                    Com_Error(
                        1, // ERR_DROP
                        "CheckWinding: non-convex\0".as_ptr() as *const c_char,
                    );
                }
                j += 1;
            }
...
pub fn AddWindingToConvexHull(w: *mut winding_t, hull: *mut *mut winding_t, normal: *mut vec3_t) {
    unsafe {
        ...
        let mut hullPoints: [vec3_t; MAX_HULL_POINTS] = [[0.0; 3]; MAX_HULL_POINTS];
        let mut newHullPoints: [vec3_t; MAX_HULL_POINTS] = [[0.0; 3]; MAX_HULL_POINTS];
        let mut hullDirs: [vec3_t; MAX_HULL_POINTS] = [[0.0; 3]; MAX_HULL_POINTS];
        ...
        Com_Memcpy(
            ...
            (numHullPoints as usize) * std::mem::size_of::<vec3_t>(),
        );
