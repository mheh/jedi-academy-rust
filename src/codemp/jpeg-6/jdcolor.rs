#![allow(non_snake_case, non_camel_case_types, non_upper_case_globals, unused_parens, dead_code, unused_mut, unused_variables)]
...
pub struct my_color_deconverter {
    ...
    // porting note: C field named `pub` renamed to `pub_` — `pub` is a Rust reserved keyword

    /* Private state for YCC->RGB conversion */
    pub Cr_r_tab: *mut c_int,   /* => table for Cr to R conversion */
    pub Cb_b_tab: *mut c_int,   /* => table for Cb to B conversion */
    pub Cr_g_tab: *mut INT32,   /* => table for Cr to G conversion */
    pub Cb_g_tab: *mut INT32,   /* => table for Cb to G conversion */
}
...
const SCALEBITS: c_int = 16; /* speediest right-shift on some machines */
const ONE_HALF: INT32 = (1 as INT32) << (SCALEBITS as u32 - 1);

// FIX(x) - ((INT32) ((x) * (1L<<SCALEBITS) + 0.5))
macro_rules! FIX {
    ($x:expr) => {
        (($x as f64 * ((1i64 << (SCALEBITS as u32)) as f64) + 0.5_f64) as INT32)
    };
}
...
unsafe fn build_ycc_rgb_table(cinfo: j_decompress_ptr) {
    ...
    (*cconvert).Cr_r_tab = ((*(*cinfo).mem).alloc_small.unwrap())(
        ...
        JPOOL_IMAGE,
        ((MAXJSAMPLE + 1) as usize) * SIZEOF!(c_int),
    ) as *mut c_int;
    ...
    (*cconvert).Cr_g_tab = ((*(*cinfo).mem).alloc_small.unwrap())(
        cinfo as j_common_ptr,
        JPOOL_IMAGE,
        ((MAXJSAMPLE + 1) as usize) * SIZEOF!(INT32),
    ) as *mut INT32;
    ...
    while i <= MAXJSAMPLE {
        ...
        *(*cconvert).Cr_r_tab.offset(i as isize) =
            RIGHT_SHIFT!(FIX!(1.40200_f64) * x + ONE_HALF, SCALEBITS) as c_int;
        /* Cb=>B value is nearest int to 1.77200 * x */
        *(*cconvert).Cb_b_tab.offset(i as isize) =
            RIGHT_SHIFT!(FIX!(1.77200_f64) * x + ONE_HALF, SCALEBITS) as c_int;
        /* Cr=>G value is scaled-up -0.71414 * x */
        *(*cconvert).Cr_g_tab.offset(i as isize) = (-FIX!(0.71414_f64)) * x;
        /* Cb=>G value is scaled-up -0.34414 * x */
        /* We also add in ONE_HALF so that need not do it in inner loop */
        *(*cconvert).Cb_g_tab.offset(i as isize) = (-FIX!(0.34414_f64)) * x + ONE_HALF;
        i += 1;
        x += 1;
    }
}
...
unsafe fn ycc_rgb_convert(
    ...
    loop {
        ...
        while col < num_cols {
            ...
            *outptr.offset(RGB_GREEN as isize) =
                *range_limit.offset((y +
                    (RIGHT_SHIFT!(*Cbgtab.offset(cb as isize) + *Crgtab.offset(cr as isize),
                                  SCALEBITS) as c_int)) as isize);
...
unsafe fn null_convert(
    ...
    loop {
        ...
        while ci < num_components {
            ...
            outptr = (*output_buf).offset(ci as isize);
            count = num_cols;
            while count > 0 {
    ...
    loop {
        ...
        while col < num_cols {
            ...
            *outptr.offset(0) = *range_limit.offset(
                (MAXJSAMPLE - (y + *Crrtab.offset(cr as isize))) as isize);  /* red */
            *outptr.offset(1) = *range_limit.offset(
                (MAXJSAMPLE - (y +                                           /* green */
                    (RIGHT_SHIFT!(*Cbgtab.offset(cb as isize) + *Crgtab.offset(cr as isize),
                                  SCALEBITS) as c_int))) as isize);
            *outptr.offset(2) = *range_limit.offset(
                (MAXJSAMPLE - (y + *Cbbtab.offset(cb as isize))) as isize);  /* blue */
...
unsafe fn start_pass_dcolor(cinfo: j_decompress_ptr) {
    /* no work needed */
}
