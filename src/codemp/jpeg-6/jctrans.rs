...
// (exe_headers.h include — translated as import below)
...
// #include "jpeglib.h"  (with JPEG_INTERNALS, also pulls jpegint.h and jmorecfg.h)
use crate::codemp::jpeg_6::jpeglib_h::*;
use crate::codemp::jpeg_6::jpegint_h::*;
use crate::codemp::jpeg_6::jmorecfg_h::*;

// Forward declarations — not required in Rust (items are visible within the
// module regardless of order).  Preserved here as documentation only.
...
pub unsafe fn jpeg_copy_critical_parameters(
    ...
    while tblno < NUM_QUANT_TBLS as c_int {
        if !(*srcinfo).quant_tbl_ptrs[tblno as usize].is_null() {
            ...
            MEMCOPY!(
                ...
                core::mem::size_of_val(&(*(*qtblptr)).quantval)
            );
    ...
    while ci < (*dstinfo).num_components {
        ...
        if tblno < 0 || tblno >= NUM_QUANT_TBLS as c_int
        ...
        if !c_quant.is_null() {
            ...
            while coefi < DCTSIZE2 as c_int {
                if (*c_quant).quantval[coefi as usize] != (*slot_quant).quantval[coefi as usize] {
...
// Porting note: the C field named `pub` (a Rust keyword) is renamed to `pub_`.
#[repr(C)]
struct my_coef_controller {
    pub_: jpeg_c_coef_controller, /* public fields */
...
unsafe extern "C" fn start_pass_coef(cinfo: j_compress_ptr, pass_mode: J_BUF_MODE) {
    let coef: my_coef_ptr = (*cinfo).coef as my_coef_ptr;
...
unsafe extern "C" fn compress_output(
    ...
    while ci < (*cinfo).comps_in_scan {
        ...
        buffer[ci as usize] = (*(*cinfo).mem).access_virt_barray.unwrap()(
            ...
            *(*coef).whole_image.add((*compptr).component_index as usize),
            (*coef).iMCU_row_num * (*compptr).v_samp_factor as JDIMENSION,
    ...
    while yoffset < (*coef).MCU_rows_per_iMCU_row {
        ...
        while MCU_col_num < (*cinfo).MCUs_per_row {
            ...
            if (*(*cinfo).entropy).encode_mcu.unwrap()(cinfo, MCU_buffer.as_mut_ptr()) == 0 {
...
unsafe fn transencode_coef_controller(
    ...
    jzero_far(buffer as *mut c_void, C_MAX_BLOCKS_IN_MCU * core::mem::size_of::<JBLOCK>());
    i = 0;
