  ...
  unsafe {
    ...
    while (*(*cinfo).master).is_dummy_pass {
      ...
      while (*cinfo).output_scanline < (*cinfo).output_height {
        ...
        ((*(*cinfo).main).process_data)(cinfo, core::ptr::null_mut() as JSAMPARRAY,
                          &mut (*cinfo).output_scanline, 0 as JDIMENSION);
     ...
     */
    (*cinfo).global_state = if (*cinfo).raw_data_out { DSTATE_RAW_OK } else { DSTATE_SCANNING };
    TRUE
  }
}
         ...
         max_lines: JDIMENSION) -> JDIMENSION
{
  unsafe {
    ...
    /* Process some data */
    row_ctr = 0;
    ((*(*cinfo).main).process_data)(cinfo, scanlines, &mut row_ctr, max_lines);
...
pub fn jpeg_start_output (cinfo: j_decompress_ptr, mut scan_number: core::ffi::c_int) -> boolean
{
  unsafe {
