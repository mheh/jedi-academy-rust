...
// Stubs from tr_types.h (external dependency)
#[repr(C)]
pub struct refEntity_t {
    // Stub - full definition in tr_types.h
    _unused: [u8; 0],
}
...
// Stub from keycodes.h (external dependency)
// (keycodes not redefined here)

// Stubs from menudef.h (external dependency)
// (menudef not redefined here)

// Stubs from ghoul2 API
#[repr(C)]
pub struct mdxaBone_t {
    // Stub - ghoul2 bone matrix
    _unused: [u8; 0],
}

#[repr(C)]
pub struct CollisionRecord_t {
    // Stub - collision record
    _unused: [u8; 0],
}

#[repr(C)]
pub struct sharedRagDollParams_t {
    // Stub - ragdoll parameters
    _unused: [u8; 0],
}

#[repr(C)]
pub struct sharedRagDollUpdateParams_t {
    // Stub - ragdoll update parameters
    _unused: [u8; 0],
}

#[repr(C)]
pub struct sharedSetBoneIKStateParams_t {
    // Stub - bone IK state parameters
    _unused: [u8; 0],
}

#[repr(C)]
pub struct sharedIKMoveParams_t {
    // Stub - IK move parameters
    _unused: [u8; 0],
}

// Stub from renderer config
#[repr(C)]
...
// STRING_POOL_SIZE determined by platform and CGAME define
// For now, define the base cases
#[cfg(target_os = "windows")]
#[cfg(feature = "CGAME")]
pub const STRING_POOL_SIZE: usize = 32*1024;

#[cfg(target_os = "windows")]
#[cfg(not(feature = "CGAME"))]
...
pub struct displayContextDef_t {
    ...
    pub AnyLanguage_ReadCharFromString: Option<unsafe extern "C" fn(*const c_char, *mut c_int, *mut qboolean) -> u32>,
    ...
    pub getTeamColor: Option<unsafe extern "C" fn(*mut *const vec4_t) -> ()>,
...
extern "C" {
    ...
    pub fn LerpColor(a: *const vec4_t, b: *const vec4_t, c: *mut vec4_t, t: f32);
    ...
    pub fn trap_AnyLanguage_ReadCharFromString(psText: *const c_char, piAdvanceCount: *mut c_int, pbIsTrailingPunctuation: *mut qboolean) -> u32;
