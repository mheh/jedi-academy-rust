...
// Placeholder types for unresolved dependencies
#[repr(C)]
...
pub type setType_t = setType_e;

// C++ section: this enum isn't used directly by the game, it's mainly for BehavEd to scan for...
//
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum playType_e {
...
pub type playType_t = playType_e;

pub const Q3_TIME_SCALE: c_int = 1;    // MILLISECONDS

pub static mut cinematicSkipScript: [c_char; 1024] = [0; 1024];

// General
extern "C" {
    pub fn Q3_TaskIDClear(taskID: *mut c_int);
    pub fn Q3_TaskIDPending(ent: *mut sharedEntity_t, taskType: taskID_t) -> qboolean;
    pub fn Q3_TaskIDComplete(ent: *mut sharedEntity_t, taskType: taskID_t);
    pub fn Q3_DPrintf(format: *const c_char, ...);

    pub fn Q3_CameraRoll(angle: f32, duration: f32);
    pub fn Q3_CameraFollow(name: *const c_char, speed: f32, initLerp: f32);
    pub fn Q3_CameraTrack(name: *const c_char, speed: f32, initLerp: f32);
    pub fn Q3_CameraDistance(distance: f32, initLerp: f32);
    ...
    pub fn Q3_SetVelocity(entID: c_int, angles: vec3_t);

    pub fn Q3_DeclareVariable(type_: c_int, name: *const c_char);
